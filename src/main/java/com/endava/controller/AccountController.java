...
import com.endava.model.Country;
import com.endava.services.UserService;
import com.endava.util.DigestUtil;
...
public class AccountController {
	...
	static {
		...
		for (Locale iterator : loc) {
			Country newCountry = new Country(iterator.getCountry(),
					iterator.getDisplayCountry());
	...
	public String signUpAccount(@Valid SignUpForm signUpForm,
		...
		if (!reCaptchaResponse.isValid()) {
			bindingResult.addError(new FieldError("signUpForm", "captcha",
					"Answer is wrong."));
		}
		...
		String text = "To verify your email address, please follow this link: http://localhost:9080/EnCoders/account/validate?code="
				+ digestEmail;
